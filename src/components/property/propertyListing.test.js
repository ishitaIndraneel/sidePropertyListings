...
describe('Property Listings', () => {
    test('renders Property Listing', async() => {
       
        const pageName = ""
            const setPageName = () => {}
            act( ()=>{
                jest.spyOn(global,'fetch').mockResolvedValue({ json: jest.fn().mockResolvedValue(fakeListings)})
            render(
      ...
      await waitFor(()=> {
        const linkElement = screen.getByTestId('listing_id_0');
